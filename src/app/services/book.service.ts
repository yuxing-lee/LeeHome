...
export class BookService extends BaseService {
  ...
  public getBooks(): Observable<book[]> {
    return this.http.get('/book', this.getRequestOptions())
      .pipe(map(ApiEndpoint.ExtractData),
            catchError(ApiEndpoint.HandleError));
  }

  public saveBook(book: object): Observable<any> {
    return this.http.post('/book', book, this.getRequestOptions())
      .pipe(map(ApiEndpoint.ExtractData),
            catchError(ApiEndpoint.HandleError));
  }

  public getBookDetail(id: string): Observable<book> {
    return this.http.get('/book/' + id, this.getRequestOptions())
      .pipe(map(ApiEndpoint.ExtractData),
            catchError(ApiEndpoint.HandleError));
  }

  public deletebook(id: string): Observable<any> {
    return this.http.delete('/book/' + id, this.getRequestOptions())
      .pipe(map(ApiEndpoint.ExtractData),
            catchError(ApiEndpoint.HandleError));
  }

  public updateBook(id: string, book: book): Observable<book> {
    return this.http.put('/book/' + id, book, this.getRequestOptions())
      .pipe(map(ApiEndpoint.ExtractData),
            catchError(ApiEndpoint.HandleError));
  }
